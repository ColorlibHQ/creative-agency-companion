/**
 * Creative Agency Companion widgets without jQuery: carousels, the portfolio filter,
 * scroll-to-top and counters, on the ColorlibUI bundle the Creative Agency theme loads.
...
(function () {
  ...
  function run() {
    ...
    UI.owl('.creativeagency-service-slides', {
      items: 3,
      loop: true,
      autoplay: true,
      smartSpeed: 800,
    ...
    UI.owl('.creativeagency-workflow-slides', {
      items: 3,
      loop: true,
      autoplay: true,
      smartSpeed: 800,
    ...
    UI.owl('.creativeagency-team-slides', {
      items: 3,
      loop: true,
      autoplay: true,
      smartSpeed: 800,
    ...
    if (window.imagesLoaded) {
      UI.imagesLoaded('.creativeagency-portfolio', function () {
        // filter items on button click
        UI.toElements('.portfolio-menu').forEach(function (menu) {
          menu.addEventListener('click', function (e) {
            var item = e.target.closest && e.target.closest('p');
            if (!item || !menu.contains(item)) return;
            UI.isotope('.creativeagency-portfolio', { filter: item.getAttribute('data-filter') });
          });
        });
        // init Isotope
        UI.isotope('.creativeagency-portfolio', {
    ...
    // MC Scripts
    if (document.querySelector('.creativeagency-subscribe-newsletter-area')) {
